...
def submit_attestation():
    ...
    if HW_PROOF_AVAILABLE:
        try:
            ...
            if not is_valid and proof_result.get('entropy_score', 0) < 0.1:
                print(f"[HW_PROOF] ❌ REJECTED: {miner} - {proof_result.get('reason')}")
                return jsonify({
                    "ok": False,
                    "error": "hardware_proof_failed",
                    "reason": proof_result.get('reason'),
                    "entropy_score": proof_result.get('entropy_score', 0),
                    "antiquity_tier": proof_result.get('antiquity_tier', 'unknown'),
                    "confidence": proof_result.get('confidence', 0)
                }), 403
            # Accept with warnings if entropy >= 0.1 but < 0.3
            elif proof_result.get('entropy_score', 0) < 0.3:
                print(f"[HW_PROOF] ⚠️  WARNING: Low entropy for {miner} ({proof_result.get('entropy_score', 0):.3f})")
            # Accept normally if entropy >= 0.3
            else:
                print(f"[HW_PROOF] ✅ ACCEPTED: {miner} - Tier: {proof_result.get('antiquity_tier', 'unknown')}")
        except Exception as e:
