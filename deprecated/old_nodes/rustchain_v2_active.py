...
# Rewards system
try:
    from rewards_implementation import (
        settle_epoch, total_balances, UNIT, PER_EPOCH_URTC,
...
if HAVE_REWARDS:
    try:
        from rewards_implementation import register_rewards
...
def lottery_eligibility():
    ...
    if not miner_id:
        return jsonify({"error": "missing miner_id"}), 400

    now = int(time.time())
    slot = now // BLOCK_TIME

    # Check if miner is enrolled in current epoch
    ok_enroll, why = check_enrollment_requirements(miner_id)
    if not ok_enroll:
        return jsonify({
            "slot": int(slot),
            "eligible": False,
            "reason": why.get('error', 'not_enrolled')
        }), 200
    ...
    return jsonify({
        ...
        "reason": "ok" if selected else "not_selected",
