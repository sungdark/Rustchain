...
ANTIQUITY_MULTIPLIERS = {
    "ppc_g3": 3.0, "g3": 3.0, "ppc750": 3.0,
    "ppc_g4": 2.5, "g4": 2.5, "ppc7400": 2.5, "ppc7450": 2.5,
    "ppc_g5": 2.0, "g5": 2.0, "ppc970": 2.0,
    "core2": 1.5, "core_2": 1.5,
    "apple_silicon": 1.2, "m1": 1.2, "m2": 1.2, "m3": 1.2, "arm64": 1.2,
    "modern": 0.8, "x86_64": 0.8, "amd64": 0.8
}
...
def get_antiquity_multiplier(arch):
    ...
    for key, mult in ANTIQUITY_MULTIPLIERS.items():
...
def get_miners():
    ...
    try:
        ...
        cur = get_db().cursor()
        cur.execute("""
            SELECT miner, device_arch, device_family, last_attest, entropy_score 
            FROM miner_attest_recent 
            ORDER BY last_attest DESC
        """)
        for row in cur.fetchall():
            miner, arch, family, last_attest, entropy = row
            multiplier = get_antiquity_multiplier(arch or "")
            miners.append({
                "address": miner,
                "device_arch": arch or "Unknown",
                "device_family": family or "Unknown",
                "last_seen": last_attest,
                "entropy_score": entropy or 0,
                "antiquity_multiplier": multiplier,
                "bonus_type": "legendary" if multiplier >= 2.5 else 
                              "ancient" if multiplier >= 2.0 else
                              "vintage" if multiplier >= 1.5 else
                              "modern_bonus" if multiplier > 1.0 else
                              "penalty" if multiplier < 1.0 else "standard"
            })
...
def get_hardware_distribution():
    ...
    try:
        ...
        cur = get_db().cursor()
        cur.execute("""
            SELECT device_arch, device_family, COUNT(*) as miner_count
            FROM miner_attest_recent
            GROUP BY device_arch, device_family
            ORDER BY miner_count DESC
        """)
        for row in cur.fetchall():
            arch, family, count = row
            multiplier = get_antiquity_multiplier(arch or "")
            hardware.append({
        ...
        return jsonify({
            "unique_architectures": len(hardware),
            "hardware": hardware
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500
