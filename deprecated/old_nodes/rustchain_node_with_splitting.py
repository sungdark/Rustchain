...
def mine_block():
    ...
    try:
        ...
        with blockchain_lock:
            ...
            if current_time - blockchain["current_block_start"] >= 120:
            ...
            if existing:
                return jsonify({
                    ...
                    "next_block_in": 120 - (current_time - blockchain["current_block_start"])
                }), 429
            ...
            return jsonify({
                ...
                "block_completes_in": 120 - (current_time - blockchain["current_block_start"])
            })
...
def index():
    ...
    <h2>With Proper Reward Splitting!</h2>
    <p>Chain ID: 2718</p>
    ...
    <p>Current Block Age: {int(time.time() - blockchain["current_block_start"])}s / 120s</p>
    {pending_details}
    <h3>How it works:</h3>
    <ul>
        <li>Miners submit proofs during 120-second window</li>
        <li>Block completes, rewards split by multipliers</li>
...
if __name__ == '__main__':
    ...
    print("⚡ WITH PROPER REWARD SPLITTING!")
    print("📍 Chain ID: 2718")
